...
async fn main() {
	...
	let periodic_timer1 =
		tokio_core::reactor::Interval::new(std::time::Duration::from_millis(2500), &handle)
			.unwrap();
	let periodic_timer2 =
		tokio_core::reactor::Interval::new(std::time::Duration::from_millis(500), &handle).unwrap();
